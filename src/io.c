...
//               6. PB2 cannot be allocated as analog input and are not used
//                PB0 - 1 ADC1-IN8-9
...
uint8_t readADC()
{
 ...
 taskENTER_CRITICAL();
  ...
  if (flagADCConversionCompleted)
  {
   ...
   HAL_ADC_Start_DMA(handleADC1, (uint32_t*)ADCValues, 9);  // Triggers a DMA Read of the MCU's internal 12-bit ADC channels
  }
 taskEXIT_CRITICAL();
